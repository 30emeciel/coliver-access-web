...
export default function ReservationList() {
  ...
  const [listRequests, listRequestsLoading, listRequestsError] = useCollectionData<TReservationRequest>(
    db.collection(`pax/${pax.sub}/requests`).orderBy("arrival_date", "asc").withConverter(TReservationRequestConverter),
    { idField: "id" },
  )
      ...
      <List
        ...
        renderItem={(item) => {
          ...
          return <List.Item actions={[
            <WorkInProgress><Button size="small"
                                    icon={<FontAwesomeIcon icon={faEdit} />}>Modifier</Button></WorkInProgress>,
            <WorkInProgress><Button danger size="small" icon={<FontAwesomeIcon
              icon={faExclamationCircle} />}>Annuler</Button></WorkInProgress>,
          ]} extra={[]}>
