...
const CancelationForm = ({
  ...
  const docDayRef = firebase
    .firestore()
    .doc(`pax/${currentUser.sub}/days/${DateTime.fromJSDate(calValue).toISODate()}`)
  ...
  const submitForm = async () => {
    ...
    var batch = db.batch()
    const request_ref = db.doc(`pax/${currentUser.sub}/requests/${requestSnap.id}`)
    batch.delete(request_ref)

    const daysQuerySnap = await db
      .collection(`pax/${currentUser.sub}/days`)
